...
static int	check_update_order(int_vec &rule1, int_vec &rule2, int_vec &update, bool is_wrong)
{
		...
		for (it_intvec it_r2 = rule2.begin(), it_r1 = rule1.begin(); it_r2 != rule2.end(); it_r1++, it_r2++) {
			if (*it == *it_r1)
			{
				for (it_intvec it2 = update.begin(); it2 != it; it2++) {
					if (*it2 == *it_r2) {
						update.insert(it2, *it);
						update.erase(it + 1);
						is_wrong = true;
						return check_update_order(rule1, rule2, update, is_wrong);
					}
				}
			}
		}
...
int main(int argc, char **argv)
{
	try {
		...
		while (std::getline(f, line))
		{
			...
			bool	is_wrong = false;
			total += check_update_order(rule1, rule2, update, is_wrong);
			if (!update.empty())
				update.clear();
		}
		std::cout << "The total total number of the middle incorrect pages is " << total << std::endl;
	}
